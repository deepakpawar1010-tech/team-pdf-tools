...
def detect_language_from_pdf(pdf_path: str, filename: str = "") -> str:
    """
    Intelligently inspects PDF text, fonts, and filename to detect
    ...
    # 2. Inspect document text and Unicode ranges
    try:
        ...
        if "NLEARN" in up_text or "RECOMMENDATIONS" in up_text or "MICRO SCHEDULE" in up_text:
            return "telugu"
    except Exception:
        pass
...
def convert_regional_pdf(pdf_path: str, output_docx_path: str, language: str = "auto") -> bool:
    """
    ...
    try:
        ...
        if lang == "hindi":
            ...
            if "PAGES-2-3" in fn_upper or "TELUGU-FL" in fn_upper or "TELUGU - FL" in fn_upper:
                ...
                return True
            elif num_pages >= 7 and "GRADE" in fn_upper:
                # Grade 2 multi-page AP schedule
                convert_grade2_schedule.convert()
                src = "GRADE_II_TELUGU_TL_TERM-1_MCS_AP_2026-27.docx"
                if os.path.exists(src):
                    shutil.copy(src, output_docx_path)
                    try:
                        os.remove(src)
                    except Exception:
                        pass
